...
TED API allows anonymous access to all services manipulating published notices (i.e. searching for, or retrieving notices already published on TED). Further information about reusing TED data is currently available on the link:https://simap.ted.europa.eu/en_GB/web/simap/developers-corner#get-published[SIMAP Developers' Corner].

However, all TED APIs manipulating notices that are not yet published, require the caller to be authenticated with an API Key.
...
You can generate an API Key by logging-in to the TED Developer Portalfootnote:portal[] and clicking on the "Manage your API Keys" button. To login to the TED Developer Portal you will need a valid link:https://webgate.ec.europa.eu/cas/[EU-Login]footnote:eu-login[{eu-login}] account. 
...
NOTE: The link:https://docs.ted.europa.eu/home/eforms/preview/index.html[Preview environment] allows developers to test their applications.

Information about eSentool, the legacy API to submit and manage notices, is available on the link:https://webgate.ec.europa.eu/fpfis/wikis/display/TEDeSender[legacy eSenders wiki].
